...
//#define WRE2P  //注释掉就是只读取数据，注意该芯片预留的E2P只能擦除烧写1000次E2P,读次数不限
...
//    
	  RF_Config();
					...
					for(i=0;i<4;i++)
					{
						...
						Step2(27+i);//擦除
						if(i==0)
						Step3(27+i,0x1234);//烧写
						if(i==1)
						Step3(27+i,0x5678);//烧写
						if(i==2)
						Step3(27+i,0x1111);//烧写
						if(i==3)
						Step3(27+i,0x2222);//烧写
						#else
						
					#endif
						if(i==0)		
						{
							e2p_tmp=Step4(27+i);//读数据
									
							if(e2p_tmp==0x1234)
							{
									views_print_line(0, "E2P27 ok");
...
void RF_Config(void)
{
	...
	//  while(1);						//gpio1为低电平,对应的TXant为低电平；GPIO2为高电平，对应的RXANT为高电平。
	  CMT2300A_ConfigGpio(
