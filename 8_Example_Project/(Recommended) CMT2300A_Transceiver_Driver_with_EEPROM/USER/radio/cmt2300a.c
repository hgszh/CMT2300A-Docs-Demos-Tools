...
BOOL CMT2300A_AutoSwitchStatus(u8 nGoCmd)
{
    ...
    case CMT2300A_GO_SLEEP:  nWaitStatus = CMT2300A_STA_SLEEP; break;
		case CMT2300A_GO_EEPROM: nWaitStatus = CMT2300A_STA_EEPROM; break;
...
{
    u8 re = CMT2300A_ReadReg(CMT2300A_CUS_INT1_CTL);

    if(0 == nMode)//gorx  GPIO2低电平，gpio1高电平  使能SWI1
			{
        re |= 0x80; //保持bit7为高,RF_SWT1_EN 置1，其他为保持读的值不变
        re &= ~0x40; //0100 0000去反1011 1111和re相&，这里就是将bit6为低，RF_SWT2_EN置0
    }
    else if(1 == nMode)//gotx gpio2高电平，gpio1为低电平 使能SWT2
			{
        re &= ~0x80; //将BIT7置低，RF_SWT1_EN置低，
        re |= 0x40;  //将bit6置高，RF_SWT2_EN置高
    }

    CMT2300A_WriteReg(CMT2300A_CUS_INT1_CTL, re);
}
...
//    CMT2300A_WriteReg(CMT2300A_CUS_INT_CLR2, nClr2);
		 ...
		 CMT2300A_WriteReg(CMT2300A_CUS_INT_CLR1, 0x3f);
    CMT2300A_WriteReg(CMT2300A_CUS_INT_CLR2, 0xff);
